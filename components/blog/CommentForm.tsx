...
import { postComment } from '@/app/blog/[slug]/actions'; // Import the action
import type { Session } from 'next-auth';
import Link from 'next/link';

interface CommentFormProps {
  articleId: string;
  session: Session | null; // Pass the session to check status
}

export default function CommentForm({ articleId, session }: CommentFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      ...
      if (!result.success) {
        ...
        // router.refresh(); // Server action revalidates, but this can force client update
      }
    });
  };
    ...
    return (
      <div className="p-4 bg-gray-100 rounded-lg text-center">
        ...
        <div className="flex justify-center gap-4">
          <Link
            href="/login?callbackUrl=CURRENT_URL_TODO" // Replace with actual article URL

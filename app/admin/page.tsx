  ...
  return (
    <div className="min-h-screen w-full bg-gray-50 p-4 md:p-8 min-w-screen flex flex-col items-center justify-center">
      <div className="max-w-7xl w-full mx-auto space-y-12">
          ...
          <div className="overflow-x-auto">
            <table className="w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {/* --- FIX: Use ternary operator for conditional rendering --- */}
                {allArticles.length > 0 ? (
                  ...
                  <tr>
                    <td colSpan={6} className="px-6 py-4 text-center text-gray-500"> No articles found. </td>
                  </tr>
                )}
              </tbody>
            </table>
        ...
        {/* --- End: All Articles Management Table --- */}
                  ...
                  bloggerApplications.map((app) => (
                    <tr key={app.id}>
                      ...
                      <td className="px-6 py-4 max-w-xs">
                        <p className="text-sm text-gray-700 truncate" title={app.reason}> {app.reason} </p>
                      </td>
                      <td className="px-6 py-4 max-w-xs">
                        <p className="text-sm text-gray-700 truncate" title={app.topics}> {app.topics} </p>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm">
                        {app.sampleUrl ? ( <a href={app.sampleUrl} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">View</a> ) : ( <span className="text-gray-400">None</span> )}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap"> <StatusBadge status={app.status} /> </td>
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <BloggerApplicationRowActions application={app as ApplicationWithUser} />
                      </td>
                    </tr>
                  ))
                ) : (
                  <tr>
                    <td colSpan={6} className="px-6 py-4 text-center text-gray-500"> No blogger applications received yet. </td>
                  </tr>
                )}
              </tbody>
            </table>
                ...
                {contactMessages.length > 0 ? (
                  contactMessages.map((message) => (
                    <tr key={message.id} className={message.isRead ? 'opacity-70' : 'font-semibold'}>
                      ...
                      <td className="px-6 py-4 text-sm text-gray-700 whitespace-normal break-words">{message.message}</td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium"> <ContactMessageRowActions messageId={message.id} isRead={message.isRead} /> </td>
                    </tr>
                  ))
                ) : (
